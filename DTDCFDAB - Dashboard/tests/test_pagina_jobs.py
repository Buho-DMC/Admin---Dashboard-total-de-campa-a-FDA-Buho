...
def test_jobs_muestra_progreso_por_lote(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        'list_jobs_de_lote',
        lambda id_lote: [
            {'id_job': 1, 'id_campana': 1, 'estado': 'exitoso'},
            {'id_job': 2, 'id_campana': 2, 'estado': 'corriendo'},
        ],
    )
    app_test = AppTest.from_file('pages/jobs.py')
    app_test.run()
...
def test_jobs_muestra_progreso_por_job_individual(monkeypatch):
    _sin_autorefresh(monkeypatch)
    llamadas_de_progreso = _capturar_texto_de_progreso(monkeypatch)
    monkeypatch.setattr(api_client, 'get_job', lambda id_job: {'id_job': 9, 'id_campana': 3, 'estado': 'exitoso'})
    app_test = AppTest.from_file('pages/jobs.py')
    app_test.run()
...
def test_jobs_muestra_boton_de_reintentar_solo_para_fallidos(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        'list_jobs_de_lote',
        lambda id_lote: [
            {'id_job': 1, 'id_campana': 1, 'estado': 'fallido'},
            {'id_job': 2, 'id_campana': 2, 'estado': 'exitoso'},
        ],
    )
    app_test = AppTest.from_file('pages/jobs.py')
    app_test.run()
...
def test_jobs_reintenta_al_hacer_click(monkeypatch):
    _sin_autorefresh(monkeypatch)
    monkeypatch.setattr(
        api_client, 'list_jobs_de_lote', lambda id_lote: [{'id_job': 1, 'id_campana': 1, 'estado': 'fallido'}]
    )
